...
export default function OverviewView({ drains, actionDrains, weatherAlert, wsConnected, flashIds, selectedId, onSelect }) {
  ...
  return (
    <div className="overview">
      ...
      <section className={`overview-panel ${panelOpen ? 'open' : 'closed'}`}>
        <button
          type="button"
        ...
        >
          <span>
            {allClear ? '조치 필요 없음' : `조치 필요 ${needCount}곳`}
          </span>
        ...
        <div className="overview-panel-content">
          <div>
            <h2 className={`ov-headline ${allClear ? 'ov-headline-clear' : ''}`}>
              {allClear ? (
                <>지금 조치가 필요한 지점은 없습니다</>
              ) : (
                <>
            ...
            <p className="ov-sub">
              전체 <strong>{drains.length}개</strong> 지점 중 현재{' '}
              <strong>{weatherModeLabel(weatherAlert?.mode)}</strong> 기준으로 차폐율{' '}
          ...
          {top.length > 0 && (
            <div>
              <h3 className="ov-urgent-head">가장 급한 지점</h3>
